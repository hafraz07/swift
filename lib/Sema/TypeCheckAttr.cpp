  ...
  if (isa<ClassDecl>(D) ||
    ...
    if (!checkObjCDeclContext(D))
      ...
      if (!accessor->isGetterOrSetter())
          diagnose(attr->getLocation(), diag::objc_observing_accessor, accessor->getDescriptiveKind());
  ...
  if (auto *accessor = dyn_cast<AccessorDecl>(D)) {
    if (!attr->isImplicit()) {
      ...
      if (auto *VD = dyn_cast<VarDecl>(accessor->getStorage()))
        Kind = VD->isLet() ? 1 : 0;
      diagnose(attr->getLocation(), diag::final_not_on_accessors, D->getDescriptiveKind(), Kind)
        .fixItRemove(attr->getRange());
      return;
    }
  }
...
static FuncDecl *findSimilarAccessor(DeclNameRef replacedVarName,
  ...
  if (results.empty()) {
    Diags.diagnose(attr->getLocation(),
                   ...
                   replacement->getDescriptiveKind(),
                   replacedVarName);
    attr->setInvalid();
    return nullptr;
  }
